...
//        click on it
        ...
        //        select the age of 3 options age  >> so get locator common between them >> then pick what you want by if
        List<WebElement> radioBtns = driver.findElements(By.xpath("//input[@name='ageGroup']"));

        for(WebElement radio:radioBtns){
            String option = radio.getAttribute("value");
            if(option.equals("15 - 50")){
                radio.click(); // click on the element
                System.out.println(option);// sout the value
                break;
            }
